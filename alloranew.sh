...
setup_telegram_bot() {
    ...
    if screen -list | grep -q "AlloraBot"; then
        ...
        log_message "⚙️ /ETHprice - *Узнать текущую цену ETH"
        log_message "🔍 /checkstatus - *Проверить статус контейнеров"
        log_message "🔄 /restartcontainer - *Перезапустить контейнеры"
        log_message "⏳ /uptimecontainer - *Показать время работы контейнеров"
        log_message "🔔 /alert - *Получить оповещения о состоянии контейнеров"
    else
